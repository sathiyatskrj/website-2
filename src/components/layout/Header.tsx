...
import { Search, Menu, Crown, ChevronDown, Users, MapPin, Calendar, FileText, Download, Trophy, X } from "lucide-react";
...
export function Header() {
  ...
  const changeFontSize = (delta: number) => {
    ...
    setFontSize((prev) => {
      ...
      if (newSize >= MIN && newSize <= MAX) return newSize;
      return prev;
    });
  };
  ...
  return (
    <header
      className={`flex flex-col w-full z-50 bg-background/90 backdrop-blur-xl border-b border-border/50 relative sticky top-0 transition-shadow duration-300 ${
        scrolled ? "shadow-[0_4px_30px_rgba(0,0,0,0.08)] dark:shadow-[0_4px_30px_rgba(0,0,0,0.5)]" : ""
      }`}
      ref={megaMenuRef}
    >
        ...
        <div className="container mx-auto px-4 lg:px-8 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 bg-black/20 px-2 py-0.5 rounded-sm">
              <button onClick={() => changeFontSize(-10)} className="px-1 hover:text-secondary transition-colors" aria-label="Decrease Font Size">A-</button>
      ...
      <div className="bg-primary border-y border-primary-foreground/10 hidden md:block relative z-20">
        <div className="container mx-auto px-4 lg:px-8">
          <nav className="flex items-center relative" aria-label="Main navigation">
            {navLinks.map((link) => (
              <div
                key={link.label}
              ...
              >
                <Link
                  href={link.href}
                  className="px-4 lg:px-6 py-4 flex items-center gap-1.5 text-[12px] lg:text-[13px] font-semibold uppercase tracking-wider text-primary-foreground/90 hover:text-white hover:bg-black/20 transition-all border-l border-primary-foreground/10 first:border-l-0"
                >
                  {link.label}
                  {link.mega && <ChevronDown className="h-3.5 w-3.5 opacity-60 group-hover:opacity-100 group-hover:rotate-180 transition-all duration-200" />}
                </Link>

                {/* Mega Menu Dropdown */}
                {link.mega && activeMegaMenu === link.label && (
                  <div className="absolute top-full left-0 w-full bg-background/97 backdrop-blur-2xl border-b border-x border-border shadow-[0_20px_60px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.7)] flex gap-12 p-8 z-50 animate-in fade-in slide-in-from-top-2 justify-center">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden border-t border-border bg-background/97 backdrop-blur-xl absolute top-full left-0 w-full z-50 shadow-2xl animate-in slide-in-from-top-2">
          <nav className="flex flex-col max-h-[80vh] overflow-y-auto" aria-label="Mobile navigation">
            {navLinks.map((link) => (
